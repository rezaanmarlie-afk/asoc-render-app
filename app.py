...
DISCUSSION_UPDATED_BY = os.getenv("DISCUSSION_UPDATED_BY", os.getenv("RTE_USER", "ASOC RTE")).strip() or "ASOC RTE"
...
APP_BUILD_VERSION = "GOV-WORKBENCH-SOURCE-DISCUSSION-WRITEBACK-2026-05-24"
...
def source_discussion_columns() -> List[Dict[str, Any]]:
    """Columns maintained on the source ASR demand sheet after stakeholder discussion.

    These columns make the operational Smartsheet itself the source of truth for
    whether a demand has already been discussed, so the default governance queue
    can hide it until the user explicitly searches for discussed/all demands.
    """
    return [
        {"title": "Stakeholder Decision", "type": "PICKLIST", "options": governance_picklist_options("Stakeholder Decision") or []},
        {"title": "Discussion Status", "type": "PICKLIST", "options": ["Not Discussed", "Discussed Already"]},
        {"title": "Discussed Date", "type": "DATE"},
        {"title": "Discussed By", "type": "TEXT_NUMBER"},
        {"title": "Last RTE Update", "type": "TEXT_NUMBER"},
    ]


def ensure_source_discussion_columns() -> Dict[str, Any]:
    """Ensure source ASR sheet has the write-back columns needed for discussions."""
    sheet = get_sheet(include="", use_cache=False)
    _, by_title = column_maps(sheet)
    created = []
    skipped = []

    for expected in source_discussion_columns():
    ...
    return {"created_columns": created, "existing_columns": skipped, "source_sheet_id": SHEET_ID}


def build_source_discussion_writeback(payload: GovernancePayload) -> Dict[str, Any]:
    decision = str(payload.stakeholder_decision or "").strip()
    is_discussed = bool(decision)
    now = datetime.now()
    summary = str(payload.discussion_summary or "").strip()
    next_action = str(payload.next_action or "").strip()
    last_update_parts = []
    if summary:
        last_update_parts.append(summary)
    if next_action:
        last_update_parts.append(f"Next Action: {next_action}")
    if payload.target_pi:
        last_update_parts.append(f"Target PI: {payload.target_pi}")
    last_update = " | ".join(last_update_parts) or f"Stakeholder decision captured: {decision}"

    return {
        "Stakeholder Decision": decision,
        "Discussion Status": "Discussed Already" if is_discussed else "Not Discussed",
        "Discussed Date": now.strftime("%Y-%m-%d") if is_discussed else "",
        "Discussed By": DISCUSSION_UPDATED_BY if is_discussed else "",
        "Last RTE Update": last_update,
    }


def update_source_discussion_status(source_row_id: int, payload: GovernancePayload) -> Dict[str, Any]:
    ensure_result = ensure_source_discussion_columns()
    writeback_values = build_source_discussion_writeback(payload)
    update_result = update_existing_smartsheet_columns(source_row_id, writeback_values)
    invalidate_sheet_cache()
    return {"template_check": ensure_result, "writeback": update_result, "values": writeback_values}
...
def create_governance_register_row(source_row_id: int, payload: GovernancePayload) -> Dict[str, Any]:
    ...
    # Write back to the original ASR demand sheet as well.
    # This marks the demand as Discussed Already so it drops out of the default meeting queue.
    source_writeback = update_source_discussion_status(source_row_id, payload)
    ...
    return {
        ...
        "source_discussion_status": source_writeback.get("values", {}).get("Discussion Status"),
...
def api_governance_demand_details(row_id: int):
    ...
    return {
        ...
        "write_back_rule": "Governance updates are written to the Governance Register and the source ASR row is marked as Discussed Already.",
        "governance_sheet_id": GOVERNANCE_SHEET_ID,
    }

@app.post("/api/governance/source-discussion-columns/repair")
def api_source_discussion_columns_repair():
    return ensure_source_discussion_columns()


@app.get("/api/governance/recommended-columns")
def api_governance_recommended_columns():
    return {"columns": governance_template_columns(), "source_discussion_columns": source_discussion_columns()}
